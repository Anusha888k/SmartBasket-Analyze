...
def generate_data(num_products=10, num_customers=100, num_transactions=500):
    """Generates fake data for products and transactions"""
    products = [fake.word() for _ in range(num_products)]  # Creating the product list
    transactions = []  # Empty list to store transactions
    ...
    df_encoded = df.explode('products').pivot_table(
        ...
        aggfunc=lambda x: 1,
        fill_value=0
    )  # Transforming into tabular form
    return df_encoded

def simple_apriori(df, min_support=0.1, min_confidence=0.5):
    """Applies the Apriori algorithm to find association rules"""
...
def perform_kmeans_with_progress(df, n_clusters=3, update_interval=5):
    """Performs K-means clustering with progress tracking"""
...
def visualize_apriori_rules(rules, top_n=10):
    """Visualizes the top Apriori rules in a 3D plot"""
...
def visualize_kmeans_clusters(df, cluster_labels):
    """Visualizes K-means clustering in 3D"""
...
def main():
    ...
    for i, labels in enumerate(kmeans_generator):
        ...
        viz.write_html(f"customer_clusters_3d_step_{i + 1}.html")
        print(f"Intermediate visual saved as 'customer_clusters_3d_step_{i + 1}.html'")
